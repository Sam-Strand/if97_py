...
def get_γ_π(π, τ):
    result = 0.0
    for i in range(len(n)): # 34
        result -= n[i] * I[i] * (7.1 - π) ** I_1[i] * (τ - 1.222) ** J[i]
    return result
...
def get_γ_ππ(π, τ):
    result = 0.0
    for i in range(len(n)): # 34
        result += n[i] * I[i] * I_1[i] * (7.1 - π) ** I_2[i] * (τ - 1.222) ** J[i]
    return result
...
def get_γ_ττ(π, τ):
    result = 0.0
    for i in range(len(n)): # 34
        result += n[i] * (7.1 - π) ** I[i] * J[i] * J_1[i] * (τ - 1.222) ** J_2[i]
    return result
...
def get_γ_πτ(π, τ):
    result = 0.0
    for i in range(len(n)): # 34
        result -= n[i] * I[i] * (7.1 - π) ** I_1[i] * J[i] * (τ - 1.222) ** J_1[i]
    return result
...
class WaterRegion:
    ...
    def enthalpy_t_p(self, t, p):
        ...
        τ = get_τ(t)
        γ_τ = get_γ_τ(π, τ)
        return 639675.036 * γ_τ
    
    def entropy_t_p(self, t, p):
        '''
        ...
        γ = get_γ(π, τ)
        γ_τ = get_γ_τ(π, τ)
        return 639675.036 / t * γ_τ - γ
    
    def volume_t_p(self, t, p):
        '''
...
if __name__ == '__main__':
    water = WaterRegion()
    print(water.enthalpy_t_p(300, 3))   # 115331.27302143839  0.115 331 273 × 10^3
